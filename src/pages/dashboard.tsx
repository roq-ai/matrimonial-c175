...
import { ProfileListPage as ShowCaseList } from 'pages/profiles';

function DashboardPage() {
  const { session } = useSession();

  const queryStat = useSWR<PaginatedInterface<QueryInterface>>(
    () => `/queries`,
    () => getQueries({ limit: 1, offset: 0 }),
  );
  ...
  const matchStat = useSWR<PaginatedInterface<MatchInterface>>(
    () => `/matches`,
    () => getMatches({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Queries',
      result: queryStat?.data?.totalCount,
      isLoading: queryStat.isLoading,
    ...
    {
      title: 'Matches',
      result: matchStat?.data?.totalCount,
      isLoading: matchStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
