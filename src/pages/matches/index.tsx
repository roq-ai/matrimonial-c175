...
type ColumnType = ColumnDef<MatchInterface, unknown>;
...
export function MatchListPage(props: MatchListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getMatches({
        relations: ['profile', 'suggestion.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MatchInterface>>(
    () => `/matches?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MatchInterface) => {
    if (hasAccess('match', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/matches/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'status', header: 'Status', accessorKey: 'status' },
    hasAccess('profile', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'profile',
          header: 'Profile',
          accessorKey: 'profile',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/profiles/view/${record.profile?.id}`}>
    ...
    hasAccess('suggestion', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.suggestion,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/matches/view/${record.id}`} passHref legacyBehavior>
                <Button
