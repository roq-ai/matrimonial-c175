...
type ColumnType = ColumnDef<SuggestionInterface, unknown>;
...
export function SuggestionListPage(props: SuggestionListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SuggestionInterface>>(
    () => `/suggestions?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: SuggestionInterface) => {
    if (hasAccess('suggestion', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/suggestions/view/${row.id}`);
    }
  };
    ...
    hasAccess('match', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/matches/view/${record.match?.id}`}>
              {record.match?.status}
            </Link>
          ),
        }
